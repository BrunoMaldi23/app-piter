  ...
  return (
    <View className="flex-1 bg-neutral-50 p-5">
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            className="mb-3 w-[48.5%] overflow-hidden rounded-2xl border border-neutral-200 bg-white"
            activeOpacity={0.75}
            onPress={() => router.push(`/seccion/${item.seccion.id}`)}
          >
            <View className="h-24 w-full bg-neutral-100">
              <ProductImage
                uri={item.seccion.imagen}
                emoji={item.seccion.emoji}
                className="h-full w-full"
                fallbackClassName="text-3xl"
                label={item.seccion.nombre}
              />
            </View>
            <View className="p-3">
